    ...
    try {
      ...
      if ($request->has('nama_nvr')) {
        ...
        if (is_array($nama_nvr)) {
          ...
          $query->where('nama_nvr', 'ilike', '%' . $nama_nvr . '%');
        }
      }


      if ($request->has('status_nvr')) {
        $status_nvr = $request->input('status_nvr');
        if (is_array($status_nvr)) {
          $query->whereIn('status_nvr', $status_nvr);
        } else {
